...
//		return getAllOfType(pack.getOwnedMember(), Package.class);
	...
	public static String getNameUnder(Class_ c) {
		String name = c.getName();
		name = name.replaceAll("([A-Z])", "_$1").toLowerCase();
		return name;
	}
	
	public static List<String> getImports(Class_ c) {
		Set<Classifier> cc = c.allParents();
		List<ElementImport> ei = c.getElementImport();
		List<PackageableElement> pe = c.getImportedMember();
		List<NamedElement> oo = c.getMember();
		List<PackageImport> pi = c.getPackageImport();
		List<Dependency> d = c.getClientDependency();
	...
	public static boolean isConstructor(Operation o) {
		Class_ c = o.getClass_();
		if (c.getName() != o.getName())
			return false;
		List<Parameter> params = o.getOwnedParameter();
		if (params.size() != 1 || !params.get(0).getType().getQualifiedName().equals(c.getQualifiedName()))
			return false;
		return true;
	}
